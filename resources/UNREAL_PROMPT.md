...
6. **时间标注**：仅在 `###` 小标题上标注时间戳，不要在正文要点中标注。格式根据视频来源平台自适应：
   - **Bilibili 来源**（`source` 中包含 `bilibili.com` 或 `b23.tv`）：`[mm:ss](SOURCE_URL?t=总秒数)`
     - 如果 `source` 已包含查询参数（如 `?p=2`），则用 `&t=总秒数` 追加
     - 示例：`### 1. 学习路径选择 [01:03](https://www.bilibili.com/video/BV1f5FozuE43?t=63)`
   - **YouTube 来源**（`source` 中包含 `youtube.com` 或 `youtu.be`）：`[mm:ss](https://www.youtube.com/watch?v=VIDEO_ID&t=总秒数s)`
     - 从 `source` 中提取 `VIDEO_ID`，注意 YouTube 的 `t=` 参数末尾有 `s` 后缀
     - 示例：`### 1. 学习路径选择 [01:03](https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=63s)`
   - **本地文件**（`source` 不是 URL）：保持纯文本 `[mm:ss]`，不加链接
     - 示例：`### 1. 学习路径选择 [01:03]`
   - 仅对内容型小标题（如知识点、操作步骤标题）添加时间戳；文档结构性标题（如"关键脚本""信号连接""场景树结构"等模板固定标题）不需要时间戳
   - 时间戳取值：从 `segments` 数据中找到该小标题内容最早被提及的时间点（`start` 字段），转换为 `mm:ss`
   - `总秒数` = 分钟数 × 60 + 秒数（例如 `01:03` → `63`，`12:30` → `750`）
   - 超过 60 分钟的视频使用 `h:mm:ss` 格式（如 `1:05:30`），`总秒数` 仍为完整秒数（如 `3930`）
7. **时间标注完整性检查**：生成文档后，逐一检查所有内容型 `###` 级标题。如果某个标题缺少时间戳，则根据该标题的关键词在 `segments` 中搜索最相关的片段，用其 `start` 时间补全时间戳。确保每个内容型 `###` 标题都有时间标注。
